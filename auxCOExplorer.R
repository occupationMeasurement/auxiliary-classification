################################################
# Der folgende Code l�dt die Hilfsklassifikation. Mithilfe einer interaktiven Darstellung k�nnen die Zusammenh�nge
# zwischen der Hilfsklassifikation und den offiziellen Klassifikationen KldB 2010 und ISCO-08 erkundet werden.
#
# Der AuxCo-Explorer erlaubt das Anlegen von Notizen zu einzelnen Hilfskategorien. Daf�r muss eine lokale Kopie der Datei https://raw.githubusercontent.com/malsch/occupationCodingAuxco/master/auxco_explorer_notes.txt erstellt werden.
# Die Pfad zu dieser lokalen Datei muss in folgender Variablen abgelegt werden.
# auxco_notes_filename <- "path/to/file"
#
# Weiterhin greift der AuxCo-Explorer auf die folgenden Dateien im Internet zur�ck:
...
# - KldB 2010 Umsteigeschl�ssel zur ISCO-08: https://statistik.arbeitsagentur.de/Statischer-Content/Grundlagen/Klassifikation-der-Berufe/KldB2010/Arbeitshilfen/Umsteigeschluessel/Generische-Publikation/Umsteigeschluessel-KldB2010-ISCO-08.xls
# - ISCO-08: http://www.statistik.at/kdb/downloads/csv/ISCO08_EN_COT_20151120_150801.txt
# - ISCO-08 (deutsche �bersetzung): http://www.statistik.at/kdb/downloads/csv/ISCO08_DE_COT_20151120_150453.txt
# - Berufsbezeichnungen aus dem BERUFENET (verf�gbar unter https://berufenet.arbeitsagentur.de/, Datenstand: 28.08.2017): http://doku.iab.de/discussionpapers/2018/dp1318_hilfsklassifikation.zip
#
...
# auxco_id 1748: KldB 61203/61204 bzw 61283/61284 wird im KldB-Baum aufgrund der Abhängigkeit von beiden Folgefragen nicht angezeigt.
# auxco_id 1749: KldB 61213/61214 bzw 61283/61284 wird im KldB-Baum aufgrund der Abhängigkeit von beiden Folgefragen nicht angezeigt.
# auxco_id 1733: ISCO 2151/3113 bzw. 2152/3114 wird im ISCO-Baum aufgrund der Abhängigkeit von beiden Folgefragen nicht angezeigt.
####################################################################################
...
# als zus�tzliche Spalte bei der KldB 2010 hinzuf�gen
...
# setnames(isco08_en, "Titel/Erl�uterungstext", "erl_text")
setnames(isco08_de, "Ebene Erl.", "erl_ebene")
names(isco08_de)[5] <- "erl_text"
# setnames(isco08_de, "Titel/Erl�uterungstext", "erl_text")
...
isco08_de[erl_text == "Beispiele f�r hier zugeordnete Berufe:", type := "occupations"]
...
tags$script("
            ...
            ")
                 ...
                 tabPanel("Kategorie l�schen", p("ToDo")),
                 tabPanel("Kategorien zusammenf�gen", p("ToDo"))
)
...
server <- function(input, output, session) {
  ...
  output$erkunden <- renderUI({
    ...
    if (!is.null(input$auxcoId) && nchar(input$auxcoId) == 4 && input$auxcoId %in% ids) {
      ...
      # folgefragen und antworten sind hier bisher sehr h�sslich umgesetzt/dargestellt.
      ...
      # Schicke an JavaScript eine Nachricht, damit dort die Suche nach der T�tigkeit im Tree beginnen kann
    ...
    return(list(
      ...
      hr(),
      p("T�tigkeit:"), p(tags$b(taetigkeit)), p(paste0(id, ": ", bezeichnung)), p("T�tigkeitsbeschreibung:", taetigkeitsbeschreibung),
      p("Abgrenzungen:"),
  ...
  session$sendCustomMessage("showHtmlPopup", paste("<html><head><title>", kldb2010[input$kldbIdClicked == kldb_id, paste(kldb_id, Titel)], "</title></head>
    ...
    <p>Zugeordnete ISCO-Kategorien laut KldB-Umsteigeschl�ssel:", kldb2010[input$kldbIdClicked == kldb_id, iscos], "</p>
